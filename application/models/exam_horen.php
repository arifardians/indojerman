...
* 
*/
class Exam_horen extends CI_Model
{
	...
	function get_avg_score_by_user_id($user_id){
		$sql = "SELECT SUM(score) / COUNT(id) AS score FROM exam_horen WHERE exam_user_id IN ( SELECT id FROM exam_user WHERE user_id = ? )"; 
		$query = $this->db->query($sql, array($user_id));
		return $query->first_row();
	}

	function get_score_records_by_user_id($user_id){
		$sql = "SELECT score FROM exam_horen WHERE exam_user_id IN ( SELECT id FROM exam_user WHERE user_id = ? )"; 
		$query = $this->db->query($sql, array($user_id));
		return $query->result_object();
	}
